...
alpha = np.zeros((dim_lattice*dim_lattice , dim_lattice*dim_lattice))

#for i in range(dim_lattice*dim_lattice):
#    for j in range(dim_lattice*dim_lattice):
#        alpha[i][j] = random.uniform(0 , 1.57)

for i in range(dim_lattice*dim_lattice):
    for j in range(dim_lattice*dim_lattice):
        alpha[i][j] = 0

alpha = np.multiply(alpha , Adj_matrix)

#alphaNode = int(input("Enter the node to add alpha: "))
#for i in range(dim_lattice*dim_lattice):
#    for j in range(dim_lattice*dim_lattice):
#        if(i != alphaNode):
#            alpha[i,j] = 0
...
i = 0
#arr_of_phases[0] =[2.4010051920304663,6.0287256858722555,1.2330827492541525,4.244941196826955,1.1455466231337177,1.2041024245383474,0.7257993703963481,0.42642983943148577,0.6253681391330541,4.305522776685203,0.6006652753115483,4.6581843865015085,2.3683353909691007,3.164821507175678,2.3261817397268465,2.749183291872449]
#above is for 4x4
...
b = 0
delay_node = int(input("Enter the node number to put alpha and delay: "))
...
def kuramoto(arr_of_phases , i):
    ...
    for j in range(nodes):
        summation += Adj_matrix.item((i,j))*np.sin((arr_of_phases[j] - arr_of_phases[i]) + alpha[i,j])
...
with open(file='alphadataMatrix.txt' , mode='a') as o:
        o.write('\n'+'Lattice dimension='+str(dim_lattice)+' '+'Time='+str(t[time-1])+','+str(time)+'\n')
        o.write('omega='+str(omega)+' '+'Coup cons='+str(coup_const)+'\n')
        o.write('Delay node='+str(delay_node)+'\n')
...
with open(file='alphadataMatrix.txt' , mode='a') as o:
        ...
        o.write('Checking the matric alpha by putting alpha=0 for all'+'\n')
